# DevFoam - G-code Generator for Foam Cutting

A comprehensive G-code generator specifically designed for hot-wire foam cutting machines. Supports 2D contour cutting, pocket cutting, and 3D surface cutting with an integrated visual CAD viewer.

## 📁 Project Organization

The code has been reorganized into a clean `src/` directory structure. See [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) for details.

**Quick Links:**
- Core modules: `src/devfoam/`
- Launch GUI: `./devfoam-gui` or `python3 -m devfoam`
- Documentation: `docs/`
- Examples: `examples/`
...
You can use the gcode_generator module directly:

```bash
cd src
python3 -m devfoam.gcode_generator example   # Simple rectangle
python3 -m devfoam.gcode_generator circle    # Circle cut
python3 -m devfoam.gcode_generator complex   # Complex pattern
```
...
from devfoam import GCodeGenerator
...
## Integration with CAD to G-code Converter

1. Load CAD file in `cad_to_gcode.py` (DXF, SVG, or JSON)
2. Configure paths interactively (optional)
3. Adjust cutting parameters
4. Generate and preview G-code
